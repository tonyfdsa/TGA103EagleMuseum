  ...
  .then(R => {
    ...
    for (i = 0; i < R.result.length; i++) {
      ...
      <div class="card-body">
       ...
       <a href="#" class="btn btn-outline-secondary" id="ticketBtn" data-id=${R.result[i].exhibitionID}>購票</a>
      </div>
      </div>
      </div>
      `;
      ...
      fetch('http://localhost:8080/TGA103eagleMuseum/ExhibitionGet', {
        ...
        .then(R => {
          ...
          for (i = 0; i < R.result.length; i++) {
            ...
            exhibitionContent += `
           <div class="card id="${R.result[i].exhibitionID}" style="padding: 0%;margin: 0.5% 0.5%; width: 19rem;">
           <img src="${R.result[i].exhibitionImgBase64}" class="card-img-top" v-on:click="exhContent"/>
           <div class="card-body">
              <h5 class="card-title">${R.result[i].exhibitionName}</h5>
              <p class="card-text">${R.result[i].exhibitionStartDate} ~ ${R.result[i].exhibitionEndDate}</p>
              <a href="#" class="btn btn-outline-secondary" id="ticketBtn1" v-on:click="exhContent">購票</a>
             </div>
            </div>
          </div>
      `;
